...
from scripts.SequentialAgent import root_agent, _timings
...
async def run():
    ...
    app_name="nps_improvement_pipeline",
    user_id="test_user",
    state={}   # rep_email/manager_email removed — unused, action_agent hardcodes them
    )
    ...
    async for event in runner.run_async(
        ...
        print(f"[{event.author}] invocation_id: {event.invocation_id}")
    ...
    print(f"\n⏱  Total pipeline time: {end_time - start_time:.2f} seconds")
    print("\n⏱  Per-agent duration (actual, via before/after_agent_callback):")
    for name, t in _timings.items():
        print(f"    {name}: {t['duration']:.2f}s")
